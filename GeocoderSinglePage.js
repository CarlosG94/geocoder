...
var DC_Name = { fieldSearch: 'name', isRequired: false };
var DC_Address = { fieldSearch: 'address', isRequired: true };


function InitializeCSVColumns() {

    DB_CSVColumns.push(DC_Name);
    DB_CSVColumns.push(DC_Address);
...
function CSVPostedRecord(CSV_Row) {
    ...
    this.Name = CSV_Row.Name;
    this.Address = CSV_Row.Address;
    this.StatusText = '';
...
function parseStepFunction(results, parser) {
    ...
    var CSV_Row = {
        ...
        Name: results.data[0][DC_Name.fieldName]
        , Address: results.data[0][DC_Address.fieldName]
        , Latitude: 0
        , Longitude: 0
    ...
    if (!CSV_Row.Name) CSV_Row.Name = '';
    if (!CSV_Row.Address) CSV_Row.Address = '';

    CSV_Row.Name = CSV_Row.Name.trim();
    CSV_Row.Address = CSV_Row.Address.trim();
...
function parseValidateCSVRow(CSV_Row) {
    ...
    if (CSV_Row.Address.length == 0) {
        ...
        CSV_Row.StatusText = 'Invalid Address';
    }
...
function displayCSVRowInitialize(CSV_Row) {
    ...
    var $TDName = $("<td id='Name_" + Row_Index + "'>" + CSV_Row.Name + "</td>");
    var $TDAddress = $("<td id='Address_" + Row_Index + "'>" + CSV_Row.Address + "</td>");
    var $TDAddressFormatted = $("<td id='AddressFormatted_" + Row_Index + "' nowrap>&nbsp;</td>");
    var $TDLatitude = $("<td id='Latitude_" + Row_Index + "'>&nbsp;</td>");
    var $TDLongitude = $("<td id='Longitude_" + Row_Index + "'>&nbsp;</td>");
...
function geocodeSingleAddress(CSV_Row) {

    if (CSV_Row.Address.length > 0) {
        geocoder.geocode({ 'address': CSV_Row.Address }, function (results, status) {
...
function ajaxPostCSVRecord_Success(CSV_Row, results) {
    ...
    if (results.length == 1) {
        ...
        CSV_Row.Latitude = location.lat().toFixed(8);
        CSV_Row.Longitude = location.lng().toFixed(8);
        CSV_Row.HasErrors = false;
        ...
        var marker = new google.maps.Marker({
            ...
            title: CSV_Row.AddressFormatted
...
// function to populate info window on top of marker
